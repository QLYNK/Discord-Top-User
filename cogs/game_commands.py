...
class GameCommands(commands.Cog):
    ...
    async def myprofile(self, interaction: discord.Interaction):
        ...
        pomodoro_profile = await pomodoro_profiles_col.find_one({"user_id": {"$in": [interaction.user.id, str(interaction.user.id)]}})
        ...
        session = await pomodoro_sessions_col.find_one({"user_id": {"$in": [interaction.user.id, str(interaction.user.id)]}})
        if session:
